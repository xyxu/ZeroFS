...
#[derive(Debug, Serialize, Clone)]
pub struct OssConfig(pub std::collections::HashMap<String, String>);
...
impl<'de> Deserialize<'de> for OssConfig {
    ...
    where
        D: Deserializer<'de>,
    {
        Ok(OssConfig(deserialize_expandable_hashmap(deserializer)?))
    }
}
...
impl Settings {
    ...
    pub fn cloud_provider_env_vars(&self) -> Vec<(String, String)> {
        ...
        if let Some(oss) = &self.oss {
            for (k, v) in &oss.0 {
                env_vars.push((format!("oss_{}", k.to_lowercase()), v.clone()));
            }
        }
        env_vars
    }
    ...
    pub fn write_default_config(path: &str) -> Result<()> {
        ...
        toml_string.push_str("\n# Optional Aliyun OSS settings\n");
        toml_string.push_str("# Use oss:// URLs with the [oss] section\n");

        // Add commented-out OSS section
        toml_string.push_str("\n# [oss]\n");
        toml_string.push_str("# access_key_id = \"${OSS_ACCESS_KEY_ID}\"\n");
        toml_string.push_str("# access_key_secret = \"${OSS_ACCESS_KEY_SECRET}\"\n");
        toml_string.push_str("# endpoint = \"https://oss-cn-hangzhou.aliyuncs.com\"  # Optional, auto-detected from URL\n");
        toml_string.push_str("# bucket = \"${OSS_BUCKET}\"  # Optional, auto-detected from URL\n");
